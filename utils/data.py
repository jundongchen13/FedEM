...
class SampleGenerator(object):
    ...
    @property
    def train_data(self):
        """create train data, only for empirical motivation verification"""
        # return four lists, which consist userId or itemId.
        ratings = self.train_ratings
        ratings['rank_latest'] = ratings.groupby(['userId'])['rating'].rank(method='first', ascending=False)
        train_emp = ratings[ratings['rank_latest'] == 1]

        train_ratings = pd.merge(train_emp, self.negatives[['userId', 'negative_samples']], on='userId')
        ...
        assert len(train_users) == len(train_items)
        assert len(negative_users) == len(negative_items)
        assert 99 * len(train_users) == len(negative_users)
        assert train_users == sorted(train_users)
        return [torch.LongTensor(train_users), torch.LongTensor(train_items), torch.LongTensor(negative_users),
